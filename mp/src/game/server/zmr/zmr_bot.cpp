...
class CZMBots : public CAutoGameSystem
{
public:
    CZMBots() : CAutoGameSystem( "ZMBots" )
    {
        m_nBots = 0;
    }
...
void CZMBots::LevelInitPostEntity()
{
    m_nBots = 0;
}
...
//-----------------------------------------------------------------------------
CBasePlayer* BotPutInServer( const char* name )
{
    char botname[64];
    Q_snprintf( botname, sizeof( botname ), "%s%02i", name, g_ZMBots.m_nBots + 1 );
    ...
    CZMPlayer* pPlayer = (CZMPlayer*)CBaseEntity::Instance( pEdict );
    pPlayer->ClearFlags();
...
// Output : 	virtual void
//-----------------------------------------------------------------------------
static void RunPlayerMove( CZMPlayer* fakeclient, const QAngle& viewangles, float forwardmove, float sidemove, float upmove, unsigned short buttons, byte impulse, float frametime )
{
    if ( !fakeclient )
        return;
...
{		
    ...
    int iTeam = ZMTEAM_HUMAN;

            
    // Ok, spawn all the bots.
    while ( --count >= 0 )
    {
        CBasePlayer* pBot = BotPutInServer( "L_Ron_Hubbard" );
...
ConCommand bot( "bot", Bot_f, "Add a bot.", FCVAR_CHEAT );
